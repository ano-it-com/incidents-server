...
class EntityRelationTypeTest extends BundleWithPostgresTestCase
{
    ...
    private function getFromDbById(string $id): ?EAVEntityRelationType
    {
        /** @var EAVEntityRelationType[] $typesFromDb */
        $typesFromDb = $this->relationTypeRepository->findBy([ (new EntityRelationTypeFilterCriteria())->where('id', '=', $id) ]);

        return count($typesFromDb) ? $typesFromDb[0] : null;
    }


    private function compareTwoTypes(EAVEntityRelationType $type1, EAVEntityRelationType $type2): void
    {
        ...
        $restrictionsFromDb = array_combine(array_map(function (EAVEntityRelationTypeRestriction $restriction) { return $restriction->getId(); }, $restrictionsFromDb), $restrictionsFromDb);

        self::assertSameSize($type1->getRestrictions(), $restrictionsFromDb);

        /**
         * @var string                           $id
         * @var EAVEntityRelationTypeRestriction $restriction
         */
    ...
    public function testAddDeleteChangeRestrictions(): void
    {
        ...
        // new
        $newRestriction = new EAVEntityRelationTypeRestriction(Uuid::uuid4()->toString());
        ...
        //updated
        /** @var EAVEntityRelationTypeRestriction $propToChange */
